...
file_handler = RotatingFileHandler(
    filename=os.path.join(LOG_DIR, "fitness_bot.log"),
    maxBytes=1024 * 1024,
    backupCount=5,
    encoding="utf-8",
)
...
ERROR_MESSAGES = {
    "uk": {
        "profile_not_found": "❌ Профіль не знайдено. Створіть профіль перед використанням цієї функції.",
        "invalid_age": "❌ Введіть коректний вік числом.",
        "invalid_height": "❌ Введіть коректний зріст у сантиметрах.",
        "invalid_weight": "❌ Введіть коректну вагу у кілограмах.",
        "save_error": "❌ Не вдалося зберегти дані. Спробуйте ще раз або зверніться до розробника.",
        "unknown_error": "❌ Сталася непередбачена помилка. Спробуйте пізніше.",
    },
    "en": {
        "profile_not_found": "❌ Profile not found. Please create a profile before using this feature.",
        "invalid_age": "❌ Please enter a valid age as a number.",
        "invalid_height": "❌ Please enter a valid height in centimeters.",
        "invalid_weight": "❌ Please enter a valid weight in kilograms.",
        "save_error": "❌ Could not save data. Try again or contact the developer.",
        "unknown_error": "❌ An unexpected error occurred. Please try again later.",
    },
}


def get_error_message(key: str, lang: str = "uk") -> str:
    return ERROR_MESSAGES.get(lang, ERROR_MESSAGES["uk"]).get(
        key, ERROR_MESSAGES["uk"]["unknown_error"]
    )
...
def log_exception(error_id: str, message: str, **context):
    logger.exception("error_id=%s | %s | context=%s", error_id, message, context)
...
def init_db():
    ...
    Функція створює основні таблиці проєкту:
    users, exercises, weight_progress, workout_progress,
...
def seed_exercises():
    """
    Планує нагадування для користувача Telegram-бота.

    Args:
        user_id (int): Telegram ID користувача.
        when (datetime): Дата та час надсилання нагадування.
        text (str): Текст нагадування.
...
async def profile_age(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        logger.warning(
            "error_id=%s | invalid_age | user_id=%s | input=%s",
            error_id,
            update.effective_user.id,
            update.message.text,
        )
        await update.message.reply_text(get_error_message("invalid_age"))
        return ASK_AGE
    await update.message.reply_text(
        "Стать (Ч/Ж)?",
...
async def profile_height(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        error_id = generate_error_id()
        logger.warning(
            "error_id=%s | invalid_height | user_id=%s | input=%s",
            error_id,
            update.effective_user.id,
            update.message.text,
        )
        await update.message.reply_text(get_error_message("invalid_height"))
        return ASK_HEIGHT
    await update.message.reply_text(
        "Вага (кг)?",
        reply_markup=ReplyKeyboardMarkup([["🔙 Назад"]], resize_keyboard=True),
...
async def profile_weight(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        error_id = generate_error_id()
        logger.warning(
            "error_id=%s | invalid_weight | user_id=%s | input=%s",
            error_id,
            update.effective_user.id,
            update.message.text,
        )
        await update.message.reply_text(get_error_message("invalid_weight"))
        return ASK_WEIGHT
...
async def error_handler(update: object, context: ContextTypes.DEFAULT_TYPE):
    ...
    logger.exception(
        "error_id=%s | unhandled_exception | update=%s",
        error_id,
        update,
    ...
    if isinstance(update, Update) and update.effective_message:
        await update.effective_message.reply_text(
            f"{get_error_message('unknown_error')}\n"
            f"Код помилки: {error_id}"
        )
